...
public class RouterHandlerFactory {
    ...
    // 需要扫描注册的Router路径
    private static volatile Reflections reflections = new Reflections("com.xxx");
    // 默认api前缀
...
//            LOGGER.debug("The requester auth information ===>Authorization:{}, Version:{}, Dev:{}",
//                    ctx.request().headers().get("Authorization"), ctx.request().headers().get("Version"), ctx.request().headers().get("Dev"));
